...
Honest out-of-fold audit of the historical archive.
    ...
    python oof_audit.py archive_prepared.csv audit.xlsx
...
Sorted worst-first, with the component driving each gap named. That list is the
practical deliverable: work the top 50 with an estimator. If the model is right
about most of them you have a business case; if it is wrong you learn which column
is missing, which is worth more than another tenth of a point of accuracy.
"""
import sys, warnings
...
parts = {c: np.zeros(len(df)) for c in T.COMPS}
...
for fold, (tr_i, te_i) in enumerate(GroupKFold(5).split(X, act, g), 1):
    ...
    regs, dm = T.fit_models(df, X, B, ci, tr, w)
    for c in T.COMPS:
        parts[c][te] = np.exp(regs[c].predict(X[te], B[te])) * (df[c + "_u"].values[te] > 0)
...
print(f"\n  out-of-fold median {np.median(ape[pl])*100:.1f}%   mean {ape[pl].mean()*100:.1f}%")
print(f"  rows beyond +/-30%: {(out['abs_gap'][pl] > 30).sum()} "
      f"({(out['abs_gap'][pl] > 30).mean()*100:.1f}%)")
print("\n  gap attributed to:")
print(out[pl & (out["abs_gap"] > 30)]["driver"].value_counts().to_string())
